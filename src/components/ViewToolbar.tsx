...
export function ViewToolbar({
  ...
  return (
    <div className="toolbar toolbar-view" onClick={(event) => event.stopPropagation()}>
      <button type="button" onClick={handleUndo} disabled={!canUndo} aria-label="Undo last change">↶</button>
      <button type="button" onClick={handleRedo} disabled={!canRedo} aria-label="Redo last change">↷</button>
      <button type="button" onClick={() => zoomFromCenter("zoomOut")} disabled={zoomIndex === 0} aria-label="Zoom out">−</button>
      <span>{zoomPercent}%</span>
      <button type="button" onClick={() => zoomFromCenter("zoomIn")} disabled={zoomIndex === maxZoomIndex} aria-label="Zoom in">+</button>
      <button type="button" onClick={onFitToContent} aria-label="Fit diagram to view">⤢</button>
    </div>
  );
}
